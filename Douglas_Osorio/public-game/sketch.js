...
socket.on('arduino-message', (arduinoMessage) => {
    ...
    let playerMove='CENTER';
    let umbralIzquierda = 341;
    let umbralDerecha = 682;

    if (actuatorValue < umbralIzquierda) {
    playerMove ='LEFT';
    
    playerMovement(player);
    } else if (actuatorValue> umbralDerecha) {
    playerMove = 'RIGHT';
    playerMovement(player);
    } else {
    playerMove = 'CENTER';
    ...
    if (btnAValue==1) {
        let encoger=0;
        encoger+=2;
        player.width=windowWidth / 10+encoger;
    }
